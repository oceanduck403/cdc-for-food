...
async def _run_with_quota(user_id: str, operation):
    reservation = await ai_quota.reserve(user_id)
    try:
        return await operation()
    except Exception:
        await ai_quota.release(reservation)
        raise


@router.post("/chat")
async def chat(body: ChatRequest, user_id: str = Depends(current_user_id)) -> dict:
    async def operation():
    ...
    return await _run_with_quota(user_id, operation)


@router.post("/survey-analysis")
async def survey_analysis(body: SurveyRequest, user_id: str = Depends(current_user_id)) -> dict:
    async def operation():
...
async def daily_suggestion(body: DailySuggestionRequest, user_id: str = Depends(current_user_id)) -> dict:
    async def operation():
...
async def food_analysis(body: FoodRequest, user_id: str = Depends(current_user_id)) -> dict:
    async def operation():
        foods = await qwen_service.analyze_food(body.imageBase64)
        return {"foods": foods, "generatedByAi": True}
    return await _run_with_quota(user_id, operation)
